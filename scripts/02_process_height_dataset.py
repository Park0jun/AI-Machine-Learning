...
def apply_temporal_smoothing(seq, sigma=1.0):
    """
    시간 축으로 가우시안 스무딩 적용
    BlazePose의 프레임 간 jitter 감소
    
    Args:
        seq: (T, 33, 3) 키포인트 시퀀스
        sigma: 스무딩 강도 (클수록 부드러움)
...
def normalize_to_hip_center(seq):
    ...
    # 엉덩이 중심 계산
    hip_center = (seq[:, LEFT_HIP] + seq[:, RIGHT_HIP]) / 2  # (T, 3)
...
def main():
    ...
    for path in paths:
        ...
        # Step 2-1: Center Normalization (위치 독립적 학습)
        centered_seq = normalize_to_hip_center(raw_seq)
        
        # Step 2-2: Temporal Smoothing (jitter 감소)
        smoothed_seq = apply_temporal_smoothing(centered_seq, sigma=1.0)
        
        # Step 2-3: Height Correction (지면 보정)
        target_seq = create_height_corrected_target(smoothed_seq)
